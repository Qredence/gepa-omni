...
When the Codex proposer is selected, the returned `info` values become materialized context for a
read-only Codex subprocess. Keep feedback concrete and bounded: include the failing output, expected
behavior, and a focused next change rather than dumping unrelated logs.
...
- `info: dict` — **free-form feedback shown to the proposer** as Actionable Side Information (ASI).
  This is the single biggest lever on mutation quality.
- For single-task runs the signature is `evaluate(candidate)`; with `dataset`/`valset`
...
- **`oa.log()`** — `import gepa.optimize_anything as oa; oa.log("landing distance:", d)` inside your
...
- Log your own per-eval record (id, score, sub-metrics, candidate hash) — you'll want it for
  analysis; a configured `run_dir` retains GEPA's run log and state, and `oa.log()` covers
